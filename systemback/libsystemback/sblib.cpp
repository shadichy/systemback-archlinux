...
void sb::ldtltr()
{
    ...
    if (lang == "auto")
    {
        if (QLocale::system().name() != "en_EN")
            tltr->load(QLocale::system(), "systemback", "_", "/usr/share/systemback/lang");
    }
    else if (lang != "en_EN")
            ...
            "\n " % tr("Operating system:") % []() -> QStr
                                                          {
                                                              ...
                                                              if (fopen(file))
                                                                  while (!file.atEnd())
                                                                  {
                                                                      ...
                                                                      if (cline.startsWith("PRETTY_NAME=\""))
                                                                          return ' ' % mid(cline, 14, cline.length() - 14);
                                                                  }
...
QStr sb::fload(cQStr &path, bool ascnt)
{
    ...
    while (!in.atEnd())
        lst.append(in.readLine());
    QStr str;
    str.reserve(ba.size() + 1), ba.clear();
    for (ushort a(lst.count()); a; --a)
        str.append(lst.at(a - 1) % '\n');
    return str;
}
...
bool sb::crtfile(cQStr &path, cQStr &txt)
{
    ...
    if (!(like(otp, {Notexist, Isfile}) && isdir(left(path, rinstr(path, "/") - 1))))
        return err();
    QFile file(path);
    if (!file.open(QFile::WriteOnly | QFile::Truncate) || file.write(txt.toUtf8()) == -1)
        return err();
    file.flush();
...
inline bool sb::crthlnk(const T1 &srclnk, const T2 &newlnk)
{
...
void sb::unlock(uchar type)
{
    ...
    {
    case Sblock:
        exist("/run/systemback.lock") ? remove("/run/systemback.lock") : exist("/var/run/systemback.lock") ? remove("/var/run/systemback.lock")
                                                                                                           : 0;
        break;
    case Alpmlock:
        ...
        break;
    default:
        exist("/run/sbscheduler.lock") ? remove("/run/sbscheduler.lock") : exist("/var/run/sbscheduler.lock") ? remove("/var/run/sbscheduler.lock")
                                                                                                              : 0;
        break;
    }
}
...
bool sb::cfgwrite(cQStr &file)
{
    ...
    return crtfile(file, "# Restore points settings\n#  storage_directory=<path>\n#  storage_dir_is_mount_point=[true/false]\n#  max_temporary_restore_points=[3-10]\n#  use_incremental_backup_method=[true/false]\n\n"
                         "storage_directory=" %
                             (cdir.isEmpty() ? sdir[0] : cdir) %
                             "\nstorage_dir_is_mount_point=" % (cdir.isEmpty() ? ismpnt ? "true" : "false" : issmfs("/.sbsystemcopy" % cdir, cdir.count('/') == 1 ? "/.sbsystemcopy" : QStr("/.sbsystemcopy" % left(cdir, rinstr(cdir, "/") - 1))) ? "false"
                                                                                                                                                                                                                                                   : "true") %
                             ...
                             "enabled=" %
                             (cdir.isEmpty() && schdle[0] ? "true" : "false") %
...
void sb::cfgread()
{
    ...
    if (sdir[0].isEmpty())
    {
        ...
        if (ismpnt != Empty)
            ismpnt = Empty;
        if (!isdir("/home/Systemback"))
            crtdir("/home/Systemback");
        if (!isfile("/home/Systemback/.sbschedule"))
            crtfile("/home/Systemback/.sbschedule");
    }
    else
    {
        if (!isdir(sdir[0] % "/Systemback") && isdir(sdir[0]) && !(ismpnt == True && issmfs(sdir[0], sdir[0].count('/') == 1 ? "/" : left(sdir[0], rinstr(sdir[0], "/") - 1))) && crtdir(sdir[0] % "/Systemback"))
            crtfile(sdir[0] % "/Systemback/.sbschedule");
        QStr cpath(QDir::cleanPath(sdir[0]));
        if (sdir[0] != cpath)
            sdir[0] = cpath, cfgupdt = true;
    }
...
uchar sb::exec(cQStr &cmd, uchar flag, cQStr &envv)
{
    ...
    else
    {
        ...
        if ((rprcnt = (bckgrnd = flag != (flag & ~Bckgrnd)) ||
                              wait ||
                              flag == (flag & ~Prgrss)
                          ? 0
                      : cmd.startsWith("mksquashfs") ? 1
                      : cmd.startsWith("xorriso")    ? 2
                      : cmd.startsWith("tar -cf")    ? 3
                      : cmd.startsWith("tar -xf")    ? 4
                                                     : 0))
            Progress = 0;
    }
    ...
    if (bckgrnd)
        return proc.startDetached(cmd) ? 0 : exit(255);
    ...
    else
    {
        ...
        while (proc.state() == QProcess::Running)
        {
            ...
            case 2:
            {
                ...
                if (Progress < (cperc = mid(pout, rinstr(pout, "%") - 5, 2).toUShort()))
                    Progress = cperc;
                break;
            }
            case 3:
                if (!ThrdLng[0])
                {
                    ...
                    if (!itmst.isEmpty())
                    {
                        ...
                        while (!in.atEnd())
                        {
                            ...
                            if (itmst.at(lcnt++) == Isfile)
                                ThrdLng[0] += fsize(sdir[2] % "/.sblivesystemcreate/" % item);
                        }
                    }
                }
            ...
            case 4:
                ...
                if (!itmst.isEmpty())
                {
                    ...
                    while (!in.atEnd())
                    {
                        QStr item(in.readLine());
                        if (itmst.at(lcnt++) == Isfile)
                            size += fsize(ThrdStr[0] % '/' % item);
                    }

                    if (Progress < (cperc = (size * 100 + 50) / ThrdLng[0]))
                        Progress = cperc;
                }
            }
        }
            ...
            for (cQStr &item : QDir(sdir[1]).entryList(QDir::Dirs | QDir::NoSymLinks | QDir::NoDotAndDotDot))
                if (!item.contains(' '))
                {
                    ...
                    if (pre.at(1).isDigit() && pre.at(2).isDigit() && pre.at(3) == '_')
                    {
                        if (pre.at(0) == 'S')
                        {
                            if (pre.at(1) == '0' || mid(pre, 2, 2) == "10")
                                pnames[mid(pre, 2, 2).toUShort() - 1] = right(item, -4);
                        }
...
ullong sb::devsize(cQStr &dev)
{
    ...
    {
        if (ioctl(odev, BLKGETSIZE64, &bsize) == -1)
            err = true;
        close(odev);
    }
...
bool sb::mkpart(cQStr &dev, ullong start, ullong len, uchar type)
{
    ...
    if (dev.length() > ((dev.contains("mmc") || dev.contains("nvme")) ? 12 : 8) || stype(dev) != Isblock)
        return err();
    ThrdType = Mkpart,
    ThrdStr[0] = dev,
    ThrdLng[0] = start,
...
#ifdef C_MNT_LIB
    if (moptns == "loop")
        return exec("mount -o loop " % dev % ' ' % mpoint) ? err() : true;
#endif
    ThrdType = Mount,
    ThrdStr[0] = dev,
    ThrdStr[1] = mpoint,
...
bool sb::setpflag(cQStr &part, cQStr &flags)
{
             ...
             { return error("\n " % tr("An error occurred while setting one or more flags on the following partition:") % "\n\n  " % part % "\n\n " % tr("Flag(s):") % ' ' % flags % fdbg(part), true); });
    {
        ...
        if (part.length() < ((ismmc || isnvme) ? 14 : 9) || stype(part) != Isblock || stype(left(part, ((ismmc || isnvme) ? 12 : 8))) != Isblock)
            return err();
    }
    ThrdType = Setpflag,
    ThrdStr[0] = part,
...
uchar sb::fcomp(cQStr &file1, cQStr &file2)
{
    ...
    return stat(bstr(file1), &fstat[0]) || stat(bstr(file2), &fstat[1])                                 ? 0
           : fstat[0].st_size == fstat[1].st_size && fstat[0].st_mtim.tv_sec == fstat[1].st_mtim.tv_sec ? fstat[0].st_mode == fstat[1].st_mode && fstat[0].st_uid == fstat[1].st_uid && fstat[0].st_gid == fstat[1].st_gid ? 2 : 1
                                                                                                        : 0;
}
...
bool sb::cpertime(cQStr &srcitem, cQStr &newitem, bool skel)
{
    ...
    struct stat istat[2];
    if (lstat(bstr(srcitem), &istat[0]))
        return err();
    bstr nitem(newitem);
    if (lstat(nitem, &istat[1]))
        return err();

    if (skel)
    {
        struct stat ustat;
        if (lstat(bstr(left(newitem, instr(newitem, "/", 21) - 1)), &ustat))
            return err();
    ...
    {
        if (chown(nitem, istat[0].st_uid, istat[0].st_gid) || ((istat[0].st_mode != (istat[0].st_mode & ~(S_ISUID | S_ISGID)) || istat[0].st_mode != istat[1].st_mode) && chmod(nitem, istat[0].st_mode)))
            return err();
    }
...
bool sb::cplink(cQStr &srclink, cQStr &newlink)
{
    ...
    struct stat sistat;
    if (lstat(bstr(srclink), &sistat) || !S_ISLNK(sistat.st_mode))
        return err();
    QStr path(rlink(srclink, sistat.st_size));
    bstr nlink(newlink);
    if (path.isEmpty() || symlink(bstr(path), nlink))
        return err();
    timeval sitimes[2];
...
bool sb::cpfile(cQStr &srcfile, cQStr &newfile, bool skel)
{
    ...
    {
        bstr sfile(srcfile);
        if (stat(sfile, &fstat) || (src = open(sfile, O_RDONLY | O_NOATIME)) == -1)
            return err();
    }
    bstr nfile(newfile);
    bool herr;
    ...
    {
        struct stat ustat;
        if (stat(bstr(left(newfile, instr(newfile, "/", 21) - 1)), &ustat))
            return err();
        fstat.st_uid = ustat.st_uid, fstat.st_gid = ustat.st_gid;
    }

    if (fstat.st_uid + fstat.st_gid && (chown(nfile, fstat.st_uid, fstat.st_gid) || (fstat.st_mode != (fstat.st_mode & ~(S_ISUID | S_ISGID)) && chmod(nfile, fstat.st_mode))))
        return err();
    utimbuf sftimes;
...
bool sb::cpdir(cQStr &srcdir, cQStr &newdir)
{
    ...
    struct stat dstat;
    if (stat(bstr(srcdir), &dstat) || !S_ISDIR(dstat.st_mode))
        return err();
    bstr ndir(newdir);
    if (mkdir(ndir, dstat.st_mode) || (dstat.st_uid + dstat.st_gid && (chown(ndir, dstat.st_uid, dstat.st_gid) || (dstat.st_mode != (dstat.st_mode & ~(S_ISUID | S_ISGID)) && chmod(ndir, dstat.st_mode)))))
        return err();
    utimbuf sdtimes;
...
void sb::edetect(QSL &elst, bool spath)
{
...

    {
        ...
        {
            QStr cline(in.readLine());
            if (cline.contains(" /var/lib/"))
                mpts.append(cline.split(' ').at(1) % '/');
        }
    }
...
bool sb::exclcheck(cQSL &elist, cQStr &item)
{
    for (cQStr &excl : elist)
        if (excl.endsWith('/'))
        {
            if (item.startsWith(excl))
                return true;
        }
        else if (excl.endsWith('*'))
        {
            if (item.startsWith(left(excl, -1)))
                return true;
        }
...
{
    struct stat istat[2];
    if (lstat(bstr(link1), &istat[0]) || lstat(bstr(link2), &istat[1]) || !(S_ISLNK(istat[0].st_mode) && S_ISLNK(istat[1].st_mode)) || istat[0].st_mtim.tv_sec != istat[1].st_mtim.tv_sec)
        return false;
...
void sb::run()
{
    ...
    case Mount:
    {
        ...
        mnt_context_set_source(mcxt, bstr(ThrdStr[0])),
            ...
            mnt_context_set_options(mcxt, !ThrdStr[2].isEmpty() ? bstr(ThrdStr[2]).data : isdir(ThrdStr[0]) ? "bind"
                                                                                                            : "noatime"),
                    ...
                    while ((prt = ped_disk_next_partition(dsk, prt)))
                        if (prt->geom.length >= 1048576 / dev->sector_size)
                        {
                            ...
                            else if (prt->type == PED_PARTITION_FREESPACE)
                            {
                                ...
                                if (fgeom[1] - fgeom[0] > 1048576 / dev->sector_size - 2)
                            ...
                            else if (!egeom.isEmpty())
                            {
                                if (prt->geom.end <= egeom.at(1))
                                    ...
                                    {
                                    case 2:
                                        if (prt->geom.length >= 3145728 / dev->sector_size)
                                            egeom.append({(prt->geom.start - egeom.at(0) < 1048576 / dev->sector_size ? egeom.at(0) : prt->geom.start), prt->geom.end + (prt->geom.end == egeom.at(1) ? 1 : 0)});
                                        break;
                                    default:
                ...
                if (!path.isEmpty() && like((path = "/dev" % right(path, -5)).length(), {8, 12}) && like(path, dlst[1]))
                {
                    ...
                    if (size > 536870911)
                    {
                        if (!fstab.isEmpty())
                        {
                                    ...
                                    while ((prt = ped_disk_next_partition(dsk, prt)))
                                        if (prt->num > 0 && prt->type != PED_PARTITION_EXTENDED)
                                        {
                                            ...
                                            if (stype(ppath) == Isblock)
                                            {
                                                ...
                                                blkid_do_probe(pr);
                                                cchar *uuid(nullptr);
                                                if (!blkid_probe_lookup_value(pr, "UUID", &uuid, nullptr) && uuid)
                                                    fchk.append("_UUID=" % QStr(uuid) % '*');
                                                blkid_free_probe(pr);
                                            }
                                        }
                            ...
                            while (!in.atEnd())
                                if (like(in.readLine().trimmed(), fchk))
                                    goto next_2;
                        }
        ...
        while (!ThrdKill && (prt = ped_disk_next_partition(dsk, prt)))
            if (QStr(ped_partition_get_path(prt)) == ThrdStr[0])
            {
                for (cQStr &flag : ThrdStr[1].split(' '))
                    if (!ped_partition_set_flag(prt, ped_partition_flag_get_by_name(bstr(flag)), 1))
                        goto err;

                if (ped_disk_commit_to_dev(dsk))
                    ThrdRslt = true;
            err:
    ...
    case Mkpart:
    {
        ...
        {
            PedPartition *crtprt(ped_partition_new(dsk, ThrdChr == Primary ? PED_PARTITION_NORMAL : ThrdChr == Extended ? PED_PARTITION_EXTENDED
            ...
            if (ped_disk_add_partition(dsk, crtprt, ped_constraint_exact(&crtprt->geom)) && ped_disk_commit_to_dev(dsk))
                ThrdRslt = true;
            ped_disk_commit_to_os(dsk);
        }
        else
            while (!ThrdKill && (prt = ped_disk_next_partition(dsk, prt)))
                ...
                {
                    PedPartition *crtprt(ped_partition_new(dsk, PED_PARTITION_NORMAL, ped_file_system_type_get("ext2"), ThrdLng[0] ? psalign(ThrdLng[0] / dev->sector_size, dev->sector_size) : psalign(prt->geom.start, dev->sector_size), ThrdLng[1] ? pealign((ThrdLng[0] + ThrdLng[1]) / dev->sector_size - 1, dev->sector_size) : prt->next && prt->next->type == PED_PARTITION_METADATA ? prt->next->geom.end
                                                                                                                                                                                                                                                                                                                                                                                              : pealign(prt->geom.end, dev->sector_size)));
                    if (ped_disk_add_partition(dsk, crtprt, ped_constraint_exact(&crtprt->geom)) && ped_disk_commit_to_dev(dsk))
                        ThrdRslt = true;
                    ped_disk_commit_to_os(dsk);
                    break;
    ...
    case Delpart:
    {
        ...
        PedDisk *dsk(ped_disk_new(dev));
        if (ped_disk_delete_partition(dsk, ped_disk_get_partition(dsk, right(ThrdStr[0], -((ismmc || isnvme) ? 13 : 8)).toUShort())))
            ped_disk_commit_to_dev(dsk), ped_disk_commit_to_os(dsk);
        return ped_disk_destroy(dsk), ped_device_destroy(dev);
    }
    case Crtrpoint:
...
bool sb::thrdcrtrpoint(cQStr &trgt)
{
...

    {
...

        {
            ...
            for (uchar a(0); a < 12; ++a)
                if (isdir(dirs[a]))
                {
                    ...
                    if (!rodir(sysitms[a], sysitmst[a], dirs[a]))
                        return out();
                }
        }
        ...
        if (isdir("./snap"))
        {
            ...
            if (!rodir("./snap", sysitms[12], sysitmst[12]))
                return out();
        }

        uint anum(0), cnum(0);
        for (uchar a(0); a < 12; ++a)
            anum += sysitmst[a].count();
        QSL rplst;
        QBA *cditms;
        QUCL *cditmst;
        uchar cperc;

        {
...

            {
                ...
                if (!(usrs << (isdir("./root") ? "" : nullptr)).last().isNull())
                {
                    ...
                    if (!rodir(homeitms.last(), homeitmst.last(), "./root", True, ilst))
                        return out();
                }
                ...
                for (schar a(usrs.count() - 2); a > -1; --a)
                {
                    ...
                    if (!rodir(homeitms[a], homeitmst[a], "./home/" % usrs.at(a), True, ilst))
                        return out();
                }
            }

            for (cQUCL &cucl : homeitmst)
                anum += cucl.count();
            Progress = 0;
            if (!crtdir(rtrgt) || (isdir("./home") && !crtdir(rtrgt % "/home")))
                return out();
            ...
            for (uchar a(0); a < usrs.count(); ++a)
            {
                ...
                if (!usr.isNull())
                {
                    ...
                    if (!(cditmst = &homeitmst[a])->isEmpty())
                    {
                        ...
                        while (!in.atEnd())
                        {
                            if (Progress < (cperc = (++cnum * 100 + 50) / anum))
                                Progress = cperc;
                            QStr item(in.readLine());
                    ...
                    if (!cpertime('.' % srcd, rtrgt % srcd))
                        return out();
                }
            }
        }

        if (isdir(rtrgt % "/home") && !cpertime("./home", rtrgt % "/home"))
            return out();
        ...
        {
            QStr rcdir('.' % cdir);
            if (isdir(rcdir) && !cpdir(rcdir, rtrgt % cdir))
                return out();
            if (ThrdKill)
                return out();
        }
    ...
    // if (isdir("./run/media"))
    // {
    //     if (!crtdir(rtrgt % "/run/media"))
    //         return out();

    //     if (isfile("./etc/fstab"))
    //     {
    //         QSL dlst(QDir("./run/media").entryList(QDir::Dirs | QDir::NoSymLinks | QDir::NoDotAndDotDot));
    //         QFile file("./etc/fstab");
    //         if (!fopen(file))
    //             return out();
    ...
    //             if (a && !fopen(file))
    //                 return out();
    //             QSL incl{"* /run/media/" % item % " *", "* /run/media/" % item % "/*"};
    ...
    //                     for (cQStr cdname : mid(cline, instr(cline, "/run/media/") + 7, instr(cline, " ", instr(cline, "/run/media/")) - instr(cline, "/run/media/") - 7).split('/'))
    //                         if (!cdname.isEmpty())
    //                         {
    //                             QStr nrpi(rtrgt % "/run/media" % fdir.append('/' % (cdname.contains("\\040") ? QStr(cdname).replace("\\040", " ") : cdname)));
    //                             if (!(isdir(nrpi) || cpdir("./run/media" % fdir, nrpi)))
    //                                 return out();
    //                         }
    ...
    //     if (!cpertime("./run/media", rtrgt % "/run/media"))
    //         return out();
    // }
    ...
    if (isdir("./var/log"))
    {
        ...
        if (!rodir(logitms, logitmst, "./var/log"))
            return out();
        ...
        if (!(cpertime("./var/log", rtrgt % "/var/log") && cpertime("./var", rtrgt % "/var")))
            return out();
    }
...
bool sb::thrdsrestore(uchar mthd, cQStr &usr, cQStr &srcdir, cQStr &trgt, bool sfstab)
{
    ...
    if (mthd != 2)
    {
        ...
        if (!usrs.last().isNull())
        {
            homeitms.append(nullptr), homeitms.last().reserve(50000),
                homeitmst.append(QUCL()), homeitmst.last().reserve(1000);
            if (!rodir(homeitms.last(), homeitmst.last(), srcdir % "/root", True, ilst))
                return false;
        }
        ...
        {
            homeitms[a].reserve(5000000), homeitmst[a].reserve(100000);
            if (!rodir(homeitms[a], homeitmst[a], srcdir % "/home/" % usrs.at(a), True, ilst))
                return false;
        }
    ...
    uchar cperc;
    auto fspchk([&trgt]
                { return dfree(trgt.isEmpty() ? "/" : trgt) > 10485760; });
    ...
    {
        {
            ...
            if (trgt.isEmpty() || (isfile("/mnt/etc/xdg/autostart/sbschedule.desktop") &&
                                   isfile("/mnt/etc/xdg/autostart/sbschedule-kde.desktop") &&
                                   isfile("/mnt/usr/bin/systemback") &&
                                   isfile("/mnt/usr/lib/systemback/libsystemback.so.1.0.0") &&
                                   // isfile("/mnt/usr/lib/systemback/sbbin") &&
                                   isfile("/mnt/usr/lib/systemback/sbscheduler") &&
                                   isfile("/mnt/usr/lib/systemback/sbsustart") &&
                                   isfile("/mnt/usr/lib/systemback/sbsysupgrade") &&
                                   isdir("/mnt/usr/share/systemback/lang") &&
                                   isfile("/mnt/usr/share/systemback/efi.tar.gz") &&
                                   isfile("/mnt/usr/share/systemback/splash.png") &&
                                   isfile("/mnt/var/lib/pacman/local/systemback-*/files") &&
                                   isfile("/mnt/var/lib/pacman/local/systemback-*/mtree")))
                elst.append({"/etc/systemback", "/etc/xdg/autostart/sbschedule*", "/usr/bin/systemback*", "/usr/lib/systemback/", "/usr/share/systemback/", "/var/lib/pacman/local/systemback-*"});
            if (sfstab)
                elst.append("/etc/fstab");
            edetect(elst);
        ...
        // {
        //     QStr srcd(srcdir % "/run/media"), trgd(trgt % "/run/media");
        ...
        if (srcdir == "/.systembacklivepoint" && isdir("/.systembacklivepoint/.systemback"))
        {
            ...
            if (!rodir(sbitms, sbitmst, "/.systembacklivepoint/.systemback"))
                return false;
            lcnt = 0;
            ...
            while (!in.atEnd())
            {
                ...
                {
                case Islink:
                    if (!((sfstab && item == "etc/fstab") || cplink("/.systembacklivepoint/.systemback/" % item, trgt % '/' % item) || fspchk()))
                        return false;
                    break;
                case Isfile:
                    if (!((sfstab && item == "etc/fstab") || cpfile("/.systembacklivepoint/.systemback/" % item, trgt % '/' % item) || fspchk()))
                        return false;
                    break;
                }
            ...
            {
                QStr item(in.readLine());
                if (sbitmst.at(lcnt++) == Isdir)
                    cpertime("/.systembacklivepoint/.systemback/" % item, trgt % '/' % item);
                if (ThrdKill)
                    return false;
            }
        }
                        ...
                        if (!like(item, excl[0]) && ((inc = !(ilst.isEmpty() || like(item, {"_.*", "_snap_", "_snap/*"}))) || !exclcheck(elst, item)))
                        {
                            ...
                            case Isdir:
                                ...
                                case Isdir:
                                    ...
                                    {
                                        QBAL sdlst;
                                        if (!(inc ? odir(sdlst, trgi, Include, ilst, item) : odir(sdlst, trgi)))
                                            return false;
                            ...
                            case Isfile:
                                ...
                                if (!(skppd || cpfile(srci, trgi) || fspchk()))
                                    return false;
                            }
                        }
...
bool sb::thrdscopy(uchar mthd, cQStr &usr, cQStr &srcdir)
{
...

    {
...

        {
            ...
            if (mthd == 5)
            {
                ...
                if (!rodir(homeitms[0], homeitmst[0], srcdir % "/etc/skel"))
                    return false;
            }
            else
            {
                if (!usrs.last().isNull())
                {
                    homeitms.append(nullptr), homeitms.last().reserve(50000),
                        homeitmst.append(QUCL()), homeitmst.last().reserve(1000);
                    if (!rodir(homeitms.last(), homeitmst.last(), srcdir % "/root", like(mthd, {2, 3}) ? True : False))
                        return false;
                }
                ...
                {
                    homeitms[a].reserve(5000000), homeitmst[a].reserve(100000);
                    if (!rodir(homeitms[a], homeitmst[a], srcdir % "/home/" % usrs.at(a), like(mthd, {2, 3}) ? True : False))
                        return false;
                }
            }
            ...
            if (isdir(srcdir % "/home") && !(isdir("/.sbsystemcopy/home") || crtdir("/.sbsystemcopy/home")))
            {
                ...
                if (!crtdir("/.sbsystemcopy/home"))
                    return false;
            }

            if (mthd > 2)
            {
                QStr mfile(isfile(srcdir % "/var/lib/dbus/machine-id") ? "/var/lib/dbus/machine-id" : isfile(srcdir % "/etc/machine-id") ? "/etc/machine-id"
                                                                                                                                         : nullptr);
                                ...
                                case Isfile:
                                    ...
                                    case Isfile:
                                        if (mthd == 5)
                                            ...
                                            {
                                            case 1:
                                                if (!cpertime(srci, trgi, !cusr.isEmpty()))
                                                    return false;
                                            case 2:
                                                goto nitem_1;
                                            }
                                    ...
                                    if (mthd == 5)
                                    {
                                        if (!cpfile(srci, trgi, !cusr.isEmpty()))
                                            return false;
                                    }
...

        {
            ...
            for (cQStr &item : QDir("/.sbsystemcopy").entryList(QDir::Dirs | QDir::NoDotAndDotDot | QDir::Files | QDir::System))
            {
                if (like(item, incl))
                {
                    ...
                    {
                        QStr srci(srcdir % '/' % item);
                        if (islink(srci) && lcomp(srci, trgi))
                            break;
                    }
                    case Isfile:
                        if (!rmfile(trgi))
            ...
            for (cQStr &item : QDir(srcdir.isEmpty() ? "/" : srcdir).entryList(QDir::Files | QDir::System))
            {
                if (like(item, incl))
                {
                    ...
                    {
                        QStr trgi("/.sbsystemcopy/" % item);
                        if (!(exist(trgi) || cplink(srci, trgi)))
                            return false;
                    }
                }
        ...
        if (mthd > 2)
            elst.append({"/etc/machine-id", "/var/lib/dbus/machine-id"});
        ...
        else
        {
            ...
            if (srcdir == "/.systembacklivepoint" && fload("/proc/cmdline").contains("noxconf"))
                elst.append({"/etc/X11/xorg.conf", "/etc/X11/xorg.conf.d/"});
        }
        ...
        for (cQStr &cdir : {"/etc/rc0.d", "/etc/rc1.d", "/etc/rc2.d", "/etc/rc3.d", "/etc/rc4.d", "/etc/rc5.d", "/etc/rc6.d", "/etc/rcS.d"})
        {
            ...
            if (isdir(srcd))
                for (cQStr &item : QDir(srcd).entryList(QDir::Files))
                    if (item.contains("cryptdisks"))
                        elst.append(cdir % '/' % item);
        }
    ...
    // {
    //     QStr srcd(srcdir % "/run/media"), trgd("/.sbsystemcopy/run/media");
    ...
    //             {
    //                 QStr trgi(trgd % '/' % item);
    //                 if (!(exist(srcdir % '/' % item) || mcheck(trgi % '/')))
    //                     recrmdir(trgi);
    //                 if (ThrdKill)
    //                     return false;
    //             }
    ...
    //                         if (!cpertime(srcdir % "/run/media/" % item, trgi))
    //                             return false;
    //                     }
    //                     else if (!cpdir(srcdir % "/run/media/" % item, trgi))
    //                         return false;
    ...
    //                                 if (!(isdir(trgi) || cpdir("/run/media" % fdir, trgi)))
    //                                     return false;
    //                             }
    ...
    if (exist("/.sbsystemcopy/var/log"))
        stype("/.sbsystemcopy/var/log") == Isdir ? recrmdir("/.sbsystemcopy/var/log") : rmfile("/.sbsystemcopy/var/log");

    if (isdir(srcdir % "/var/log"))
    {
        if (!crtdir("/.sbsystemcopy/var/log"))
            return false;
        QBA logitms;
        QUCL logitmst;
        logitms.reserve(20000), logitmst.reserve(1000);
        if (!rodir(logitms, logitmst, srcdir % "/var/log"))
            return false;
        ...
        if (!logitmst.isEmpty())
        {
            ...
            while (!in.atEnd())
            {
                ...
                {
                case Isdir:
                    if (!crtdir("/.sbsystemcopy/var/log/" % item))
                        return false;
                    break;
                case Isfile:
                    if (!like(item, excl) && !(item.contains('.') && isnum(right(item, -rinstr(item, ".")))))
                    {
                        ...
                        crtfile(trgi);
                        if (!cpertime(srcdir % "/var/log/" % item, trgi))
                            return false;
                    }
                }
            ...
            {
                QStr item(in.readLine());
                if ((logitmst.at(lcnt++) == Isdir && !cpertime(srcdir % "/var/log/" % item, "/.sbsystemcopy/var/log/" % item)) || ThrdKill)
                    return false;
            }
        }

        if (!(cpertime(srcdir % "/var/log", "/.sbsystemcopy/var/log") && cpertime(srcdir % "/var", "/.sbsystemcopy/var")))
            return false;
    }
    ...
    if (srcdir == "/.systembacklivepoint" && isdir("/.systembacklivepoint/.systemback"))
    {
        ...
        while (!in.atEnd())
        {
            ...
            {
            case Islink:
                if (!(item == "etc/fstab" || cplink("/.systembacklivepoint/.systemback/" % item, "/.sbsystemcopy/" % item)))
                    return false;
                break;
            case Isfile:
                if (!(item == "etc/fstab" || cpfile("/.systembacklivepoint/.systemback/" % item, "/.sbsystemcopy/" % item)))
                    return false;
            }
        ...
        {
            QStr item(in.readLine());
            if ((sbitmst.at(lcnt++) == Isdir && !cpertime("/.systembacklivepoint/.systemback/" % item, "/.sbsystemcopy/" % item)) || ThrdKill)
                return false;
        }
    }
...
bool sb::thrdlvprpr(bool iudata)
{
    ...
    if (!(crtdir(sdir[2] % "/.sblivesystemcreate/.systemback") && crtdir(sdir[2] % "/.sblivesystemcreate/.systemback/etc")))
        return false;

    if (isdir("/etc/udev"))
    {
        if (!crtdir(sdir[2] % "/.sblivesystemcreate/.systemback/etc/udev"))
            return false;
    ...
    if ((isfile("/etc/fstab") && !cpfile("/etc/fstab", sdir[2] % "/.sblivesystemcreate/.systemback/etc/fstab")) || !cpertime("/etc", sdir[2] % "/.sblivesystemcreate/.systemback/etc"))
        return false;
    if (exist("/.sblvtmp"))
        stype("/.sblvtmp") == Isdir ? recrmdir("/.sblvtmp") : rmfile("/.sblvtmp");
    if (!crtdir("/.sblvtmp"))
        return false;
    ...
    //         return false;
    // }
    // else if (exist("/run/media/.sblvtmp"))
    ...
    //                     if (!cdname.isEmpty())
    //                     {
    //                         QStr sbli("/run/media/.sblvtmp/run/media" % fdir.append('/' % (cdname.contains("\\040") ? QStr(cdname).replace("\\040", " ") : cdname)));

    //                         if (!isdir(sbli))
    //                         {
    //                             if (!cpdir("/run/media" % fdir, sbli))
    //                                 return false;
    //                             ++ThrdLng[0];
    //                         }
    //                     }
    ...
    if (exist("/var/.sblvtmp"))
        stype("/var/.sblvtmp") == Isdir ? recrmdir("/var/.sblvtmp") : rmfile("/var/.sblvtmp");
    if (ThrdKill)
        return false;
    uint lcnt;

    {
        ...
        if (!rodir(varitms, varitmst, "/var"))
            return false;
        if (!(crtdir("/var/.sblvtmp") && crtdir("/var/.sblvtmp/var")))
            return false;
        ++ThrdLng[0];
            ...
            while (!in.atEnd())
            {
                ...
                if (exist(srci))
                {
                    if (!(like(item, excl[0], Mixed) || like(item, excl[1], All) || like(item, excl[2]) || exclcheck(elst, item)))
                        ...
                        {
                        case Islink:
                            if (!cplink(srci, "/var/.sblvtmp/var/" % item))
                                return false;
                            ++ThrdLng[0];
                            break;
                        case Isdir:
                            if (!crtdir("/var/.sblvtmp/var/" % item))
                                return false;
                            ++ThrdLng[0];
                            break;
                        case Isfile:
                            if (issmfs("/var/.sblvtmp", srci) && !crthlnk(srci, "/var/.sblvtmp/var/" % item))
                                return false;
                            ++ThrdLng[0];
                        }
    ...
    if (isdir("/snap"))
    {
        if (exist("/snap/.sblvtmp"))
            stype("/snap/.sblvtmp") == Isdir ? recrmdir("/snap/.sblvtmp") : rmfile("/snap/.sblvtmp");
        if (ThrdKill)
            return false;
        QBA snapitms;
        QUCL snapitmst;
        snapitms.reserve(10000), snapitmst.reserve(500);
        if (!rodir("/snap", snapitms, snapitmst))
            return false;
        if (!(crtdir("/snap/.sblvtmp") && crtdir("/snap/.sblvtmp/snap")))
            return false;
        ++ThrdLng[0];
        ...
        if (!snapitmst.isEmpty())
        {
            ...
            while (!in.atEnd())
            {
                ...
                if (exist(srci) && !like(item, excl))
                    ...
                    case Islink:
                    case Isfile:
                        if (!crthlnk(srci, "/snap/.sblvtmp/snap/" % item))
                            return false;
                        ++ThrdLng[0];
                        break;
                    case Isdir:
                        if (!crtdir("/snap/.sblvtmp/snap/" % item))
                            return false;
                        ++ThrdLng[0];
                    }
            ...
            while (!in.atEnd())
            {
                ...
                if (snapitmst.at(lcnt++) == Isdir)
                {
                    ...
                    if (exist(sbli) && !cpertime("/snap/" % item, sbli))
                        return false;
                }
        ...
        if (!cpertime("/snap", "/snap/.sblvtmp/snap"))
            return false;
    }
    ...
    if (uhl)
    {
        if (exist("/home/.sbuserdata"))
            stype("/home/.sbuserdata") == Isdir ? recrmdir("/home/.sbuserdata") : rmfile("/home/.sbuserdata");
        if (!(crtdir("/home/.sbuserdata") && cpdir("/home", "/home/.sbuserdata/home")))
            return false;
    }
        ...
        if (!usr.isNull())
        {
            ...
            if (uhl)
            {
                if (usr.isEmpty())
                {
                    ...
                    if (exist(usdir))
                        stype(usdir) == Isdir ? recrmdir(usdir) : rmfile(usdir);
                    if (!(crtdir(usdir) && crtdir(usdir.append("/root"))))
                        return false;
                }
                else
                {
            ...
            if (!rodir(useritms, useritmst, srcd, !iudata ? True : False))
                return false;
