...
systemback::systemback() : QMainWindow(nullptr, Qt::FramelessWindowHint), ui(new Ui::systemback)
{
    ...
    for(QWdt wdgt : QWL{ui->statuspanel, ui->scalingbuttonspanel, ui->buttonspanel, ui->resizepanel}) 
        wdgt->hide();
    for(QWdt wdgt : QWL{ui->dialogpanel, ui->windowmaximize, ui->windowminimize, ui->windowclose}) 
        wdgt->setBackgroundRole(QPalette::Foreground);
    for(QWdt wdgt : QWL{ui->function3, ui->windowbutton3, ui->windowmaximize, ui->windowminimize, ui->windowclose}) 
    ...
    for(QWdt wdgt : QWL{ui->windowbutton3, ui->buttonspanel, ui->windowminimize, ui->windowclose}) 
    ...
    dialog = getuid() + getgid() ? 306 : [this] {
            ...
            if(args.count() == 2 && args.at(1) == "schedule" && [] {
                ...
                return 300;
            else if (!((sislive = sb::isfile("/cdrom/LiveOS/squashfs.img")) || sb::lock(sb::Alpmlock)))
                return 301;
    ...
    else
    {
        ...
        for(QWdt wdgt : QWL{ui->substatuspanel, ui->subpanel}) 
            wdgt->setBackgroundRole(QPalette::Background);
        for(QWdt wdgt : QWL{ui->function2, ui->function4, ui->windowbutton2, ui->windowbutton4}) 
            wdgt->setForegroundRole(QPalette::Base);
        ...
        if(! sstart)
        {
            ...
            for(QWdt wdgt : QWL{ui->windowmaximize, ui->scalingbutton, ui->homepage1, ui->homepage2, ui->email, ui->donate}) 
                connect(wdgt, SIGNAL(Mouse_Pressed()), this, SLOT(mpressed()));
    ...
    connect(ui->wpanel, &pnlevent::Resize, [this] {
            ...
            for(QWdt wdgt : QWL{ui->function1}) wdgt->resize(wndw->width(), wdgt->height());
    ...
    if(! utblck)
    {
        ...
        if(! utimer.isActive())
        {
            ...
            if(! sstart)
            {
                ...
                for(QWdt wdgt : QWL{ui->restorepanel, ui->copypanel, ui->livepanel, ui->repairpanel, ui->excludepanel, ui->includepanel, ui->schedulepanel, ui->aboutpanel, ui->licensepanel, ui->settingspanel, ui->choosepanel}) wdgt->move(ui->sbpanel->pos()), wdgt->setBackgroundRole(QPalette::Background);
                        ...
                        else
                        {
                            ...
                            else
                            {
                                schar indx(
                                    ...
                                    sb::lang == "vi_VN" ? lst.indexOf("Tiếng Việt") :
            ...
            next_1:
                ...
                if(! sislive)
                {
                    ickernel = [this] {
                            ...
                            return !sb::exec("sh -c \"lsmod | grep 'overlay'\"");
                        }();
...
QStr systemback::guname()
{
    if(! uchkd && (! ui->admins->count() || ui->admins->currentText() == "root"))
    {
...

        {
            ...
            if(sb::fopen(file))
            {
                QSL incl{"*:x:1000:*", "*:x:1001:*", "*:x:1002:*", "*:x:1003:*", "*:x:1004:*", "*:x:1005:*", "*:x:1006:*", "*:x:1007:*", "*:x:1008:*", "*:x:1009:*", "*:x:1010:*", "*:x:1011:*", "*:x:1012:*", "*:x:1013:*", "*:x:1014:*", "*:x:1015:*"};
...
void systemback::wbreleased()
{
    if(ui->buttonspanel->isVisible())
    {
        ...
        {
            if(ui->windowclose->foregroundRole() == QPalette::Highlight) 
                ui->windowclose->setForegroundRole(ui->windowclose->backgroundRole() == QPalette::Background ? QPalette::Text : QPalette::Base);
        }
...
{
    (ui->buttonspanel->isVisible() ?
        minside(ui->windowminimize) ? 
            ui->windowminimize : 
            ui->windowclose->isVisible() &&
            minside(ui->windowclose) ? 
                ui->windowclose : 
                ui->windowmaximize : 
            ui->scalingbutton->isVisible() &&
            minside(ui->scalingbutton) ? 
                ui->scalingbutton : 
                minside(ui->homepage1) ? 
                    ui->homepage1 :
                    minside(ui->homepage2) ? 
                        ui->homepage2 : 
                        minside(ui->email) ? 
                            ui->email : 
...
void systemback::abtreleased()
{
    if(ui->homepage1->foregroundRole() == QPalette::Highlight)
        ui->homepage1->setForegroundRole(QPalette::Text),
        sb::exec("su -c \"xdg-open https://github.com/shadichy/systemback-archlinux &\" " % guname(), sb::Bckgrnd);
    else if(ui->homepage2->foregroundRole() == QPalette::Highlight)
        ui->homepage2->setForegroundRole(QPalette::Text),
...
void systemback::repair()
{
    ...
    if(! mthd)
    {
        ...
        dialog = sb::exec("arch-chroot /mnt sh -c \"grub-install --force --target=" % grub.arch % grub.name % (grub.isEFI ? "--efi-directory=/boot/efi --bootloader-id=SB --recheck" : ui->grubreinstallrepair->currentText() == "Auto" ? sb::gdetect("/mnt") : ui->grubreinstallrepair->currentText()) % ";" % ugrubcmd() % "\"") ? 318 : 208;
        // mlst.move(0, 1);
    ...
    else
    {
        ...
        if(! ppipe)
        {
            if(! (sb::isdir("/.systembacklivepoint") || sb::crtdir("/.systembacklivepoint"))) sb::rename("/.systembacklivepoint", "/.systembacklivepoint_" % sb::rndstr()), sb::crtdir("/.systembacklivepoint");

            if(! sb::mount("/cdrom/LiveOS/squashfs.img", "/.systembacklivepoint", "loop")) return dialogopen(334);
        ...
        if(rv)
        {
            if(ui->grubreinstallrepair->isVisibleTo(ui->repairpanel) && ! (ui->grubreinstallrepair->isEnabled() && ui->grubreinstallrepair->currentText() == tr("Disabled")))
            {
                // QSL mlst{"dev", "dev/pts", "proc", "sys"};
                // for(cQStr &bpath : (sb::mcheck("/run") ? mlst << "/run" : mlst)) sb::mount('/' % bpath, "/mnt/" % bpath);
                sb::exec("arch-chroot /mnt " % ugrubcmd());
                if(intrrpt) return exit();
                if((fcmp < 2 || ! (ui->autorepairoptions->isChecked() || ui->grubreinstallrepair->currentText() == "Auto")) && sb::exec("arch-chroot /mnt grub-install --force --target=" % grub.arch % grub.name % (grub.isEFI ? "--efi-directory=/boot/efi --bootloader-id=SB --recheck" : (ui->autorepairoptions->isChecked() || ui->grubreinstallrepair->currentText() == "Auto" ? sb::gdetect("/mnt") : ui->grubreinstallrepair->currentText())))) 
...
void systemback::systemcopy()
{
    ...
    if(! ppipe)
    {
        if(pname == tr("Live image"))
        {
            ...
            if(! sb::mount("/cdrom/LiveOS/squashfs.img", "/.systembacklivepoint", "loop")) return err(ui->userdatafilescopy->isVisibleTo(ui->copypanel) ? 332 : 333);
            if(intrrpt) return err();
    ...
    if(ui->usersettingscopy->isVisibleTo(ui->copypanel))
    {
        if(sb::exec("arch-chroot /.sbsystemcopy sh -c \"for rmuser in $(grep :\\$6\\$* /etc/shadow | cut -d : -f 1) ; do [ $rmuser = " % guname() % " ] || [ $rmuser = root ] || userdel $rmuser ; done\"")) return err();
        QStr nfile;
...
void systemback::livewrite()
{
...

    {
        ...
        if(sb::exec("dd if=/usr/lib/syslinux/bios/mbr.bin of=" % ldev % " conv=notrunc bs=440 count=1") || ! sb::setpflag(ldev % ((ismmc || isnvme) ? "p" : nullptr) % '1', "boot lba")
...
void systemback::on_partitionrefresh2_clicked()
{
    ...
    if(! ui->partitionsettingspanel1->isVisibleTo(ui->copypanel))
        ui->partitionsettingspanel2->isVisibleTo(ui->copypanel) ? 
    ...
    if(crrnt && ! (prvs && crrnt->row() == prvs->row()))
    {
        ...
        case sb::Extended:
        {
            if(ui->partitionsettingspanel2->isHidden()) 
                ui->partitionsettingspanel1->isVisible() ? 
            ...
            for(ushort a(crrnt->row() + 1) ; a < ui->partitionsettings->rowCount() && ((type == sb::Extended && ui->partitionsettings->item(a, 0)->text().startsWith(sb::left(dev, (dev.contains("mmc") || dev.contains("nvme")) ? 12 : 8)) && sb::like(ui->partitionsettings->item(a, 8)->text().toInt(), {sb::Logical, sb::Emptyspace})) || (type != sb::Extended && ui->partitionsettings->item(a, 0)->text().startsWith(dev))) ; ++a)
            {
                ...
                if(! mntcheck)
                {
                    ...
                    if(! mpt.isEmpty())
                    {
                        ...
                        if(((ui->point1->isEnabled() || ui->pointpipe11->isEnabled()) && sb::sdir[0].startsWith(mpt)) || sb::like(mpt, {'_' % tr("Multiple mount points") % '_', "_/cdrom_", "_/LiveOS/image_"}))
                            mntcheck = true;
        ...
        default:
            if(ui->partitionsettingspanel1->isHidden()) 
                ui->partitionsettingspanel2->isVisible() ? 
                    ui->partitionsettingspanel2->hide() : 
                    ui->partitionsettingspanel3->hide(),
                    ui->partitionsettingspanel1->show();
            ...
            else
            {
                ...
                else
                {
                    ...
                    if(sb::sdir[0].startsWith(mpt) || sb::like(mpt, {'_' % tr("Multiple mount points") % '_', "_/cdrom_", "_/LiveOS_"}) || [&mpt] {
                            if(sb::isfile("/etc/fstab"))
                            {
                                QFile file("/etc/fstab");
...
QStr systemback::mkisocmd(cQStr &path, cQStr &out)
{
    QStr cmd("mkisofs");
    QStr mbr;
    QStr gpt;
    QStr arg("-eltorito-platform 0xEF -eltorito-boot efi.img");
    if(! sb::execsrch("mkisofs")) cmd = "genisoimage";
    if(sb::execsrch("xorriso"))
        cmd = "xorriso -as mkisofs",
        mbr = "-isohybrid-mbr \"" % path % "/isolinux/isohdpfx.bin\"",
        gpt = "-isohybrid-gpt-basdat",
        arg = "-e efi.img";
    return cmd % " -V sblive -cache-inodes -J -R " % mbr % " -c isolinux/boot.cat -b isolinux/isolinux.bin -iso-level 3 -no-emul-boot -boot-load-size 4 -boot-info-table -eltorito-alt-boot " % arg % " -no-emul-boot " % gpt % " -o \"" % out % ".iso\" \"" % path % "\"";
}

bool systemback::cpdir(cQStr &src, cQStr &newdir)
{
    if(!(sb::isdir(src) && (sb::isdir(newdir) || sb::crtdir(newdir)))) return false;
    sb::exec("bash -c \"cp -r '" % src % "'/* '" % newdir % "'\"");
    return true;
}
...
void systemback::on_livenew_clicked()
{
    ...
    // {
        ...
        // if ((xmntry && !sb::remove("/usr/lib/initcpio/hooks/sbnoxconf")) || !sb::remove("/usr/lib/initcpio/hooks/sbfinstall"))
        //     return err();
    // }
...

    {
        ...
        {
            if(sb::isdir("/home/.sbuserdata")) 
            {
                ide.append(" /home/.sbuserdata/home");
                // elist.append(" -e /home/.sbuserdata/home/*/.cache");
                sb::exec("sh -c \"rm -rf /home/.sbuserdata/home/*/.cache\"");
            }
            if(sb::isdir("/root/.sbuserdata")) 
            {
                ide.append(" /root/.sbuserdata/root");
                sb::exec("rm -rf /root/.sbuserdata/root/.cache");
            }
        }
        ...
        sb::unlock(sb::Alpmlock);
        sb::unlock(sb::Sblock);
        if(sb::exec("mksquashfs" % ide % " \"" % sb::sdir[2] % "\"/.sblivesystemcreate/.systemback /media/.sblvtmp/media /var/.sblvtmp/var \"" % sb::sdir[2] % "\"/.sblivesystemcreate/LiveOS/squashfs.img " % (sb::xzcmpr ? "-comp xz " : "-comp zstd ") % " -info -b 1M -no-duplicates -no-recovery -always-use-fragments " % elist, sb::Prgrss)) return err(311);
        sb::lock(sb::Alpmlock);
        sb::lock(sb::Sblock);
    }
...

    {
        ...
        if(sb::fsize(sb::sdir[2] % "/.sblivesystemcreate/LiveOS/squashfs.img") > 4294967295) rpart = "root=LABEL=SBROOT ";
        ...
        if(xmntry)
            grxorg = "menuentry \"" % tr("Boot Live without xorg.conf file") % "\" {\n  set gfxpayload=keep\n  linux /boot/vmlinuz-" % kname % " " % rpart % "boot=live rd.live.image noxconf quiet splash" % prmtrs % "\n  initrd " % grub_ucode % "/boot/initramfs-" % kname % ".img\n}\n\n", srxorg = "label noxconf\n  menu label " % tr("Boot Live without xorg.conf file") % "\n  kernel /boot/vmlinuz-" % kname % "\n  append " % rpart % "boot=live rd.live.image initrd=" % syslinux_ucode % "/boot/initramfs-" % kname % ".img noxconf quiet splash" % prmtrs % "\n\n";
        if(
            (sb::isfile("/usr/share/systemback/efi-amd64.bootfiles") && sb::exec("tar --zstd -xf /usr/share/systemback/efi-amd64.bootfiles -C \"" % sb::sdir[2] % "\"/.sblivesystemcreate --no-same-owner --no-same-permissions")) || !(
                ...
                sb::crtfile(sb::sdir[2] % "/.sblivesystemcreate/boot/grub/loopback.cfg", "menuentry \"" % tr("Boot Live system") % "\" {\n  set gfxpayload=keep\n  linux /boot/vmlinuz-" % kname % " " % rpart % "boot=live rd.live.image iso-scan/filename=$iso_path quiet splash" % prmtrs % "\n  initrd " % grub_ucode % "/boot/initramfs-" % kname % ".img\n}\n\nmenuentry \"" % tr("Boot system installer") % "\" {\n  set gfxpayload=keep\n  linux /boot/vmlinuz-" % kname % " " % rpart % "boot=live rd.live.image iso-scan/filename=$iso_path finstall quiet splash" % prmtrs % "\n  initrd " % grub_ucode % "/boot/initramfs-" % kname % ".img\n}\n\nmenuentry \"" % tr("Boot Live in safe graphics mode") % "\" {\n  set gfxpayload=keep\n  linux /boot/vmlinuz-" % kname % " " % rpart % "boot=live rd.live.image iso-scan/filename=$iso_path xforcevesa nomodeset quiet splash" % prmtrs % "\n  initrd " % grub_ucode % "/boot/initramfs-" % kname % ".img\n}\n\n" % grxorg % "menuentry \"" % tr("Boot Live in debug mode") % "\" {\n  set gfxpayload=keep\n  linux /boot/vmlinuz-" % kname % " " % rpart % "boot=live rd.live.image iso-scan/filename=$iso_path" % prmtrs % "\n  initrd " % grub_ucode % "/boot/initramfs-" % kname % ".img\n}\n")) ||
    ...
    if(sb::autoiso)
    {
        ...
        if(isize + 52428800 < sb::dfree(sb::sdir[2]))
        {
            ...
            if (!sb::copy("/usr/lib/syslinux/bios/isohdpfx.bin", sb::sdir[2] % "/.sblivesystemcreate/isolinux/isohdpfx.bin"))
                return err();
            ui->progressbar->setValue(0);
            ...
            if (sb::exec(mkisocmd(sb::sdir[2] % "/.sblivesystemcreate", sb::sdir[2] % "/" % ifname)))
            {
