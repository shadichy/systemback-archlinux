        ...
        if(! sstart)
        {
            ...
            connect(ui->scalingbuttonspanel, &pnlevent::Mouse_Leave, [this] {
                    ...
                    else
                    {
                        ...
                        if(fscrn)
                            utimer.stop(), hide(),
                            sb::exec("/usr/bin/systemback finstall", sb::Wait);
                        else
                            sb::exec("/usr/bin/systemback", sb::Bckgrnd);
...
void systemback::repair()
{
    ...
    else
    {
        ...
        if(! ppipe)
        {
            ...
            if(! sb::mount("/cdrom/live/airootfs.sfs", "/.systembacklivepoint", "loop")) return dialogopen(334);
...
void systemback::systemcopy()
{
...

    {
        ...
        for(cQStr &vals : msort)
        {
            ...
            if(fstype.length() > 2 || !sb::exec("sh -c \"[ -z $(lsblk -nr -o fstype " % part % ") ]\""))
            {
    ...
    if(! ppipe)
    {
        if(pname == tr("Live image"))
        {
            ...
            if(! sb::mount("/cdrom/live/airootfs.sfs", "/.systembacklivepoint", "loop")) return err(ui->userdatafilescopy->isVisibleTo(ui->copypanel) ? 332 : 333);
            if(intrrpt) return err();
...
QStr systemback::rkernel()
{
    ...
    return (sb::isfile(kdir % "/kernelbase") ? sb::execSTDOUT("sh -c \"cat " % kdir % "/kernelbase | tr -d '\\n' \"\"") : sb::execSTDOUT("sh -c \"pacman -Qo " % kdir % "/vmlinuz | awk '{print $5}' | tr -d '\\n' \""));
}
...
{
    return (sb::execsrch("reboot") ? "reboot" : "sh -c '$(ps -o cmd --no-headers -p 1) 6'");
}
            ...
            for(ushort a(crrnt->row() + 1) ; a < ui->partitionsettings->rowCount() && ((type == sb::Extended && ui->partitionsettings->item(a, 0)->text().startsWith(sb::left(dev, (dev.contains("mmc") || dev.contains("nvme")) ? 12 : 8)) && sb::like(ui->partitionsettings->item(a, 8)->text().toInt(), {sb::Logical, sb::Emptyspace})) || (type != sb::Extended && ui->partitionsettings->item(a, 0)->text().startsWith(dev))) ; ++a)
            {
                ...
                if(! mntcheck)
                {
                    ...
                    if(! mpt.isEmpty())
                    {
                        ...
                        if(((ui->point1->isEnabled() || ui->pointpipe11->isEnabled()) && sb::sdir[0].startsWith(mpt)) || sb::like(mpt, {'_' % tr("Multiple mount points") % '_', "_/cdrom_", "_/live/image_"}))
                            mntcheck = true;
        ...
        default:
            ...
            else
            {
                ...
                else
                {
                    ...
                    if(sb::sdir[0].startsWith(mpt) || sb::like(mpt, {'_' % tr("Multiple mount points") % '_', "_/cdrom_", "_/live_"}) || [&mpt] {
                            if(sb::isfile("/etc/fstab"))
                            {
                                QFile file("/etc/fstab");
...
void systemback::on_mountpoint_currentTextChanged(cQStr &arg1)
{
    ...
    if(ui->mountpoint->isEnabled())
    {
        ...
        else
        {
            ...
            if(arg1.isEmpty() || (arg1.length() > 1 && arg1.endsWith('/')) || sb::like(arg1, {'_' % ompt % '_', "_/bin_", "_/sbin_", "_/etc_", "_/lib_", "_/lib32_", "_/lib64_"}) || (ui->usersettingscopy->isVisible() && arg1.startsWith("/home/")) || (arg1 != "/boot/efi" && ui->partitionsettings->item(ui->partitionsettings->currentRow(), 10)->text().toULongLong() < 268435456) || (grub.isEFI && mpt == "/boot/efi" && arg1 != "/boot/efi") || (nohmcpy[0] && mpt == "/home" && arg1 != "/home") || (mpt == "SWAP" && arg1 != "SWAP")
                || (arg1 != "SWAP" && [&] {
                        for(ushort a(0) ; a < ui->partitionsettings->rowCount() ; ++a)
...
void systemback::on_repairmountpoint_currentTextChanged(cQStr &arg1)
{
    ...
    else if(! arg1.startsWith("/mnt") || sb::like(arg1, {"*/_", "_/mnt/bin_", "_/mnt/sbin_", "_/mnt/etc_", "_/mnt/lib_", "_/mnt/lib32_", "_/mnt/lib64_"}) || (arg1.length() > 5 && sb::issmfs("/", "/mnt")) || sb::mcheck(arg1 % '/'))
    {
...
QStr systemback::mkisocmd(cQStr &path, cQStr &out)
{
    ...
    if(sb::execsrch("xorriso"))
        ...
        arg = "-e efi.img";
    return cmd % " -V sblive -cache-inodes -J -R " % mbr % " -c isolinux/boot.cat -b isolinux/isolinux.bin -iso-level 3 -no-emul-boot -boot-load-size 4 -boot-info-table -eltorito-alt-boot " % arg % " -no-emul-boot " % gpt % " -o \"" % out % "\" \"" % path % "\"";
}
...
void systemback::on_livenew_clicked()
{
    ...
    auto err([this](ushort dlg = 0) {
            ...
            for(cQStr &dir : {"/.sblvtmp", "/snap/.sblvtmp", "/var/.sblvtmp", "/home/.sbuserdata", "/root/.sbuserdata"})
...

    {
        ...
        sb::unlock(sb::Alpmlock);
        sb::unlock(sb::Sblock);
        if(sb::exec("mksquashfs" % ide % " \"" % sb::sdir[2] % "\"/.sblivesystemcreate/.systemback /var/.sblvtmp/var \"" % sb::sdir[2] % "\"/.sblivesystemcreate/live/airootfs.sfs " % (sb::xzcmpr ? "-comp xz " : "-comp zstd ") % " -info -b 1M -no-duplicates -no-recovery -always-use-fragments " % elist, sb::Prgrss)) return err(311);
        sb::lock(sb::Alpmlock);
        sb::lock(sb::Sblock);
    }
    ...
    pset(19, " 3/3"),
    sb::Progress = -1;
    for(cQStr &dir : {"/.sblvtmp", "/var/.sblvtmp"}) sb::remove(dir);
...

    {
        ...
        if(sb::fsize(sb::sdir[2] % "/.sblivesystemcreate/live/airootfs.sfs") > 4294967295) rpart = "root=LABEL=SBROOT ";
        ...
        if(!(
                sb::copy("/usr/share/systemback/splash.png", grubdir % "/splash.png") &&
                sb::crtfile(grubdir % "/grub.cfg", grubconf) &&
                sb::crtfile(grubdir % "/theme.txt", grubtheme) &&
                sb::crtfile(grubdir % "/loopback.cfg", grublbconf)) || intrrpt) 
            return err();
        if(
            !sb::remove(sb::sdir[2] % "/.sblivesystemcreate/.systemback") ||
            intrrpt ||
            (sb::isdir(sb::sdir[2] % "/.sblivesystemcreate/userdata") &&
                !sb::remove(sb::sdir[2] % "/.sblivesystemcreate/userdata")) ||
            intrrpt)
            return err();
    }
